...
# -----------------------------------------------------------------------------
# 对称量化配置：offset设为0，GROUP_SIZE=32
...
# -----------------------------------------------------------------------------
# Symmetric quantization: removed offset parameter
def run_moe_via_grouped_gemv(x, w13, w13_scale, w2, w2_scale, topk_ids, topk_weights):
    ...
    Symmetric quantization: no offset needed
    """
    ...
    # 调用 standalone gemv (symmetric quantization, no offset)
...
def run_test_bs1():
    ...
    HIDDEN_SIZE = 2048
    INTER_SIZE = 768
    GROUP_SIZE = 32  # Changed from 128 to 32
    ...
    # npu_fused_experts internally creates zero offsets for antiquant_offset
    ...
    # Symmetric quantization: no offset parameters
